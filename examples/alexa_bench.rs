...
type Rustls_Connector = HttpsConnectorRustls;
...
//Struct for Rustls
pub struct HttpsConnectorRustls {
    ssl: TlsClient,
}

impl HttpsConnectorRustls {
    fn new(ssl: TlsClient) -> HttpsConnectorRustls {
        HttpsConnectorRustls {
            ssl: ssl,
        }
    }
}

impl NetworkConnector for HttpsConnectorRustls {
...
fn create_rustls_config() -> TlsClient {
    ...
    let certfile = fs::File::open(filename).expect("cannot open certificate file");
    let mut reader = BufReader::new(certfile);
    tls.root_store.add_pem_file(&mut reader).unwrap();
    let tls = Arc::new(tls);
    let client = TlsClient {cfg: tls};
    client//port
}

///Funcn :  make_https_rustls_connector
///         Converts an OpenSSLClient to an
///         HttpsConnector.
fn make_https_rustls_connector(ssl_client: TlsClient) -> Rustls_Connector {
    let https_connector_rustls = HttpsConnectorRustls::new(ssl_client);
    https_connector_rustls
}
...
fn exec_rustls_client(config: Rustls_Connector, site: &str) -> f64{
    let cert_check = Instant::now();
    let port = 443;
    let scheme = "https";
    config.connect(site, port, scheme);
    let cert_check_timings = duration_nanos(Instant::now().duration_since(cert_check));
    cert_check_timings
}
...
fn website_bench(site: &str, exp: &Experiment) -> Vec<f64> {
    ...
    match *exp {
       Experiment::OpenSSL => {
        ...
        Experiment::Rustls => {
        	let client_creation = Instant::now();
            let client = create_rustls_config();
            let client_creation_time = duration_nanos(Instant::now().duration_since(client_creation));
            client_times.push(client_creation_time);

            let connector = make_https_rustls_connector(client); 

            let cert_verfn_times = exec_rustls_client(connector, site);
...
fn main() {
    ...
    println!("Hyper - Rustls Measurements");
